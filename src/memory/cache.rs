...
//! operations sweep the whole cache (write back dirty lines, invalidate), so
//! whole-cache initialization loops in the PROM (`IP32processorTCI`
//! `CACH_PD|C_IST` over every index) behave correctly regardless of exact
//! hardware index bits. Hit-type operations (`C_HWB`, `C_HWBINV`, `C_HINV`)
//! act on the exact addressed line. The R5000's "faked" secondary cache
//! aliases to the primary D cache (`CACH_SD` selects the same lines).
//!
...
impl DCache {
    ...
    /// Sweep whole cache: write back all dirty lines and invalidate.
    fn writeback_and_invalidate_all(&mut self, mem: &mut MemoryMap) {
    ...
    pub fn cache_instruction(&mut self, mem: &mut MemoryMap, op_field: u32, phys: u32) {
        ...
        match op {
            // Index mirror invalidation: C_IINV / C_IWBINV (0) and C_IST (2):
            // the PROM sweeps the whole cache (IP32processorTCI).
            0 | 2 => self.writeback_and_invalidate_all(mem),
            ...
            // Index Load Tag (1), Create Dirty Exclusive (3), Hit Set
            // Virtual (7): no functional effect required for the PROM.
            _ => {}
        }
    }
